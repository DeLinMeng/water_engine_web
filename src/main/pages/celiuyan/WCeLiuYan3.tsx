...
import { BaseItem, WCeLiuYanType3 } from "../../Home.type";
import { initWCeLiuYan3 } from "../../Containts";
...
interface Props {}
const WCeLiuYan3: FC<Props> = (props) => {
  const reducer = (
    state: WCeLiuYanType3,
    action: { id?: NonNullable<BaseItem>["id"]; payload: WCeLiuYanType3 }
  ) => {
    return { ...state, ...action?.payload };
  };

  const [state, dispatch] = useReducer(reducer, initWCeLiuYan3);
  ...
  const onSubmit = useCallback(async () => {
    ...
    const q = await CeiLiuYan.getQs3({
      b: state.b.count!,
      C: state.C.count!,
      h: state.h.count!,
      g: state.g.count!,
...
export default memo(WCeLiuYan3);
